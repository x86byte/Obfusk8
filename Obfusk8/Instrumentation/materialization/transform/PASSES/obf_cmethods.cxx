...
NOOPT
    #define OBF_METHOD_(ret_type, func_name, params, method_body) \
        ret_type func_name params { \
            ...
            uintptr_t entropy = (uintptr_t)_ReturnAddress(); \
            volatile unsigned int _d_seed = OBF_MBA_ADD( \
                (int)(__rdtsc() & 0xFFFFFFFFULL) ^ (int)(std::time(nullptr)) ^ (int)entropy, \
                _obf_global_opaque_seed ^ (int)(__COUNTER__ + __LINE__) \
            ); \
            _obf_global_opaque_seed = (int)_d_seed; \
            \
            ...
            vm_s.pc = ((unsigned int)__TIME__[4] ^ (unsigned int)_obf_global_opaque_seed) \
                      % obf_vm_engine::VM_HANDLER_TABLE_SIZE; \
            \
